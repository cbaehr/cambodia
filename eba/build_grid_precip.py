...
rasters = os.listdir(path+"/precip")

temp = getValuesAtPoint(indir=path+'/precip', rasterfileList=rasters, pos=empty_grid, lon='longitude', lat='latitude', cell_id='cell_id')

full_grid = pd.concat([empty_grid['cell_id'].reset_index(drop=True), temp.drop(['cell_id','x','y'], axis=1).reset_index(drop=True)], axis=1)

full_grid.to_csv(path+"/precip_grid.csv",index=False)
