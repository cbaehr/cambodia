...
gov_grid = pd.read_csv(path+"/governance_grid.csv", nrows=100)
print(gov_grid.shape)
gov_grid.drop(["cell_id"], axis=1, inplace=True)

mine_grid = pd.read_csv(path+"/mine_grid.csv", nrows=100)
print(mine_grid.shape)
mine_grid.drop(["cell_id"], axis=1, inplace=True)

main_grid = pd.read_csv(path+"/full_grid_new.csv", nrows=100)
...
treatment_grid = pd.read_csv(path+"/irrigation_grid.csv", nrows=100)
...
with open(output+"/pre_panel_corrected_irrigation.csv") as f, open(output+"/panel_corrected_irrigation.csv", "w") as f2:
	a=f2.write("cell_id,year,longitude,latitude,province_number,province_name,district_number,district_name,commune_number,commune_name,plantation,concession,protected_area,dist_to_city,dist_to_road,popdensity2000,dist_to_minecasualty,treecover,ndvi,trt1km,trt2km,trt3km,trt4km,trt5km,temperature,precip\n")
