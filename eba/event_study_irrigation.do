...
egen max_trt = max(trt_irrigation), by(cell_id)
drop if max_trt==0
drop max_trt
...
drop province_number province_name district_number district_name commune_name distance_to_city distance_to_road treecover trt_irrigation1km trt_irrigation2km trt_irrigation3km trt_irrigation4km trt_irrigation5km trt_irrigation
...
estout model1 using "$results/model1_irrigation.tsv", replace cells("b se") mlabels(,none)

reghdfe ndvi ib40.time_to_trt, absorb(year cell_id) cluster(year cell_id) pool(10)
estimates store model2
estout model2 using "$results/model2_irrigation.tsv", replace cells("b se") mlabels(,none)

reghdfe ndvi ib40.time_to_trt temperature precip, absorb(year cell_id) cluster(year commune_number) pool(10)
estimates store model3
estout model3 using "$results/model3_irrigation.tsv", replace cells("b se") mlabels(,none)

reghdfe ndvi ib40.time_to_trt temperature precip, absorb(year cell_id) cluster(year cell_id) pool(10)
estimates store model4
estout model4 using "$results/model4_irrigation.tsv", replace cells("b se") mlabels(,none)
