...
import delimited "$data/panel_corrected_irrigation.csv", clear

gen trt_irrigation_overall = trt1km_irrigation + trt2km_irrigation + trt3km_irrigation + trt4km_irrigation + trt5km_irrigation

egen max_trt_overall = max(trt_irrigation_overall), by(cell_id)
...
save "$data/panel_corrected_irrigation_formatted.dta", replace
