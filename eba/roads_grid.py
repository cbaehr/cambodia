...
roads_treatment = gpd.read_file(path+"/pid/pid_roadsandbridges.geojson")
roads_treatment.crs = "epsg:4326"
roads_treatment = roads_treatment[["end_year", "mrb_dist", "geometry"]]
roads_treatment["mrb_dist"] = roads_treatment["mrb_dist"].astype(int)

###

for i in [1000, 2000, 3000, 4000, 5000]:
	temp = roads_treatment.loc[roads_treatment["mrb_dist"]==i]
	...
	treatment_grid_temp.columns = ["treatment_roads"+str(i)+"_"+str(j) for j in range(1999, 2019)]
	if i==1000:
...
treatment_grid.reset_index(drop=True).to_csv(path+"/roads_grid.csv", index=False)
